...
# ---------------------------------------------------------------------------
# Terranova / Prospa .pt2 EFNMR image
# ---------------------------------------------------------------------------
...
def load_pt2(path: str) -> np.ndarray:
    ...
    ------
    ValueError
        If the LAER marker is not found or the data length is not a perfect square.
    """
    ...
    # Try common square NMR image sizes (powers of two)
    for n in [16, 32, 64, 128, 256]:
        if n_total == n * n and np.isfinite(arr).all() and arr.max() > 0:
            return arr.reshape(n, n)

    # Fallback: any perfect square
